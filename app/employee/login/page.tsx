...
function LoginContent() {
  ...
  useEffect(() => {
    if (!storeUser) return;
    const dest = redirect ?? REDIRECT_MAP[storeUser.role] ?? "/kitchen/orders";
    router.replace(dest);
  }, [storeUser, redirect, router]);
    ...
    <div className="min-h-screen bg-gray-950 py-8 px-4 flex items-center justify-center">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        ...
        <p className="text-center text-sm text-gray-600 mt-5">
          Chưa có tài khoản?{" "}
          <Link href="/employee/register" className="text-[#E4002B] font-semibold hover:underline">Đăng ký</Link>
        </p>
      </motion.div>
    </div>
  );
